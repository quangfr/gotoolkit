(function () {
    ...
    function sanitizeLucideIconName(value, fallback = "sparkles") {
        const token = String(value || "").trim().toLowerCase();
        if (!token) return fallback;
        return /^[a-z0-9-]{1,40}$/.test(token) ? token : fallback;
    }
    ...
    function buildSuperpowerBadges(superpowers, catalog) {
        if (!Array.isArray(superpowers) || !superpowers.length) return null;
        ...
        superpowers.forEach(id => {
            ...
            if (!label) return;
            const icon = sanitizeLucideIconName(match?.icon || "sparkles");

            const badge = document.createElement("span");
            badge.className = "share-card-template";
            ...
            badge.appendChild(document.createTextNode(String(label)));
            meta.appendChild(badge);
            count += 1;
        });
        return count ? meta : null;
    }
    ...
    async function createShareCardElement(cardInfo, target = "_self") {
        ...
        if (String(record?.notionPageId || "").trim()) {
            const iconEl = document.createElement("i");
            iconEl.setAttribute("data-lucide", "notebook");
        ...
        title.appendChild(document.createTextNode(String(preview.tabName || "")));

        const description = document.createElement("p");
        description.className = "share-card-desc";
        description.textContent = (preview.lines || []).join(" · ") || "(sans contexte)";
        ...
        if (superpowerHtml) card.appendChild(superpowerHtml);
    ...
    function buildCreateCard() {
        ...
        iconEl.style.verticalAlign = "text-bottom";
        title.appendChild(iconEl);
        title.appendChild(document.createTextNode("Créer un nouveau document"));
        card.appendChild(title);
        card.addEventListener("click", () => {
    ...
    async function buildSavedCard(record, catalog) {
        ...
        const description = document.createElement("p");
        description.className = "share-card-desc";
        description.textContent = (preview.lines || []).join(" · ") || "Complète le contexte pour l'afficher ici.";
