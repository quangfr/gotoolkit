...
5. Add local drafts via `window.goToolkitCapsuleDrafts` (IndexedDB helpers) when offline persistence is desired.
   - Reuse the shared IndexedDB helpers (`public/js/document-storage.js`) so every module talks to the same `go-toolkit` database (`capsule-drafts`, `share-history`, `documents-settings` stores) without re-implementing the boilerplate. Call `goToolkitDocStore.createStore("capsule-drafts")` (or `"share-history"` etc.) to `get`/`set` JSON objects and rely on the helper’s fallback to no-ops when IndexedDB is unavailable.
...
- Modules that need more than localStorage should reuse the shared document manager (`public/js/document-parser.js` + `public/js/document-storage.js`). `document-parser.js` builds its own `gotoolkit-documents` database (stores: `documents`, `chunks`, `keyword_meta`) for embedded docs/knowledge chunks, while `document-storage.js` exposes wrappers around the `go-toolkit` database that stores drafts, share history, and desktop settings. This unified approach keeps all app surfaces pointing at the same IndexedDB schema so knowledge, drafts, and share metadata stay aligned across tabs and modules.
